...
public class CombiUniverse {
    ...
    // 총 n개의 번호공을 r개를 뽑아 만들수 있는 순열
    static void n2r(int[] arr, int[] output, boolean[] visited, int depth, int n, int r) {
        if (depth == r) {
            for(int i =0 ; i < r; i++) {
        ...
        for (int i=0; i<n; i++) {
            if (visited[i] != true) {
                visited[i] = true;
                output[depth] = arr[i];
                n2r(arr, output, visited, depth + 1, n, r);
                visited[i] = false;;
            }
        }
    }
